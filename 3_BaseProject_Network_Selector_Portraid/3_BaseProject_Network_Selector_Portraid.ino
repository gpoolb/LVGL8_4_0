...
// Compiler Adjust
// a) board:      ESP32 Dev Module
// b) Flash Mode: DIO
// c) Partition squeme: Huge App (3MB APP no OTA 1MB SPIFF)
...
//const char *ntpServer = "srvad01.cicy.mx";
...
//  prefs.putUInt("WIFI_FLAG", flag);
  EEPROM.writeString(EEPROM_ADDR_WIFI_CREDENTIAL, flag == 1 ? ssidpw : "");
//  prefs.putString("WIFI_CREDENTIAL", flag == 1 ? ssidpw : "\0");
  EEPROM.commit();
  Serial.print("saveWIFICredentialEEPROM() -> ");
  Serial.print("flag: ");
  Serial.print(flag);
  Serial.print(" ssidpw: ");
...
//  int wifiFlag = prefs.getUInt("WIFI_FLAG", 0);  // Note: Key name is limited to 15 chars.

  String wifiCredential = EEPROM.readString(EEPROM_ADDR_WIFI_CREDENTIAL);
//  String wifiCredential = prefs.getString("WIFI_CREDENTIAL","\0");  // Note: Key name is limited to 15 chars.
  Serial.print("loadWIFICredentialEEPROM() -> ");
  Serial.print("wifiFlag: ");
  Serial.print(wifiFlag);
  Serial.print(" wifiCredential: ");
...
//  prefs.end();
  ...
  if (wifiFlag == 1 && wifiCredential.length() != 0 && wifiCredential.indexOf(" ") != -1) {
    ...
    if (sscanf(wifiCredential.c_str(), "%s %s", preSSIDName, preSSIDPw) > 0) {
      ...
      networkConnector();
      Serial.print("loadWIFICredentialEEPROM() -> ssidName: ");
...
static void timerForNetwork(lv_timer_t *timer) {
  ...
  switch (networkStatus) {
    ...
    case NETWORK_CONNECTED:
      lv_label_set_text(timeLabel, "Sin Internet!    " LV_SYMBOL_CLOSE);
      showingFoundWiFiList();
      updateLocalTime();
      break;
...
static void buildSettings() {
  ...
  lv_obj_set_size(settings, 220, 270);  // tft.width() - 100, tft.height() - 40
  lv_obj_align(settings, LV_ALIGN_TOP_MID, 0, 30);
  ...
  lv_obj_set_size(wfList, 180, 210);  // tft.width() - 140, 210
